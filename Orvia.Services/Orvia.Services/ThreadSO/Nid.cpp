			...
			std::string  _pathDebugFile			)
{
	...
	if (debugMode == 1) {
		...
		snprintf(buffer, STR_SIZE, "|CONSTRUCTION| Nid deviceBalance : %s, deviceRFID : %s, baudrate : %i, nbDataBits : %i, parity : %i, nbStopBits : %i, SN Balance : %s, SN RFID : %s, Min poids oeuf : %f, Max poids oeuf : %f, Min poids poule : %f, Max poids poule : %f", 
...
int  Nid::PalmiOnBalance(std::string * sError)
{
	...
	if (debugMode == 1) {
		...
		snprintf(buffer, STR_SIZE, "Nid::PalmiOnBalance ret = %i, SN antenne = %s, tag RFID = %s, poids balance = %f",
...
int  Nid::refresh(std::string* sError)
{
	...
	if (debugMode == 1) {
		...
		snprintf(buffer, STR_SIZE, "Nid::refresh ret = %i, SN antenne = %s, first tag RFID = %s, poids balance = %f poids oeuf total min = %f, poids oeuf total max = %f, poids poule total min = %f, poids poule total max = %f, poids total min = %f, poids total max = %f",
